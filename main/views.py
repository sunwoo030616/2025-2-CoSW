...
class LostItemCreateView(APIView):
    ...
    def post(self, request):
        ...
        # AI 서버 붙으면 실제 embedding 받아서 저장
        vlm_vec = generate_fake_embedding()

        lost_item = UserLostItem.objects.create(
            user=request.user,
            description=description,
            original_image_url=image_url,
            vlm_embedding=vlm_vec
        )

        # PostgreSQL pgvector 기반 즉시 매칭
        matches = vector_similarity(vlm_vec, top_k=3)
